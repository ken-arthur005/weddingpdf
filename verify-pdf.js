...
function isUsableLink(annotation, expected) {
  if (!annotation.get(PDFName.of('P')) || !annotation.get(PDFName.of('Contents'))) return false;

  const action = annotation.get(PDFName.of('A'));
  if (!action || !action.toString().includes('/Type /Action')) return false;

  if (expected.target.type === 'internal') {
    return action.toString().includes('/S /GoTo');
  }

  return action.toString().includes('/S /URI')
    && action.toString().includes(expected.target.url);
}
...
async function verifyPdf() {
  ...
  for (const expected of LINK_TARGETS) {
    ...
    const matchingAction = annotations.some(annotation => isUsableLink(annotation, expected));
    if (!matchingAction) {
